...
static void cvSuperLUMTReinit(CVodeMem cvode_mem);

static void cvSuperLUMTFreeGlobalData(CVodeMem cvode_mem);
...
int CVSuperLUMT(void *cvode_mem, int num_threads, int n, int nnz)
{
  ...
  cv_mem->cv_lreinit = cvSuperLUMTReinit;
  cv_mem->cv_lfreeglobal = cvSuperLUMTFreeGlobalData;
...
#else
  pdgstrf2(superlumt_options, AC, perm_r, L, U, Gstat, &info, &slumt_data->global_data);
#endif
...
static void cvSuperLUMTReinit(CVodeMem cvode_mem)
{
	...
	cvsls_mem = (CVSlsMem)cvode_mem->cv_lmem;
	slumt_data = (SLUMTData)cvsls_mem->s_solver_data;

	if (slumt_data->superlumt_options) pxgstrf_finalize(slumt_data->superlumt_options, slumt_data->s_AC);

	if (slumt_data->s_L->Store) Destroy_SuperNode_SCP(slumt_data->s_L);
	slumt_data->s_L->Store = NULL;
	if (slumt_data->s_U->Store) Destroy_CompCol_NCP(slumt_data->s_U);
	slumt_data->s_U->Store = NULL;

	if (slumt_data->s_A->Store) SUPERLU_FREE(slumt_data->s_A->Store);
	slumt_data->s_A->Store = NULL;
}
...
static void cvSuperLUMTFreeGlobalData(CVodeMem cvode_mem)
{
	...
	if (slumt_data->global_data)
		free(slumt_data->global_data);
	slumt_data->global_data = NULL;
}
