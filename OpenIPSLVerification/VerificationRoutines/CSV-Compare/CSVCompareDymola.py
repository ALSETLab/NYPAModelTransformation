...
ReportDir = os.path.abspath(os.path.join(ReportDir, os.pardir))
ReportDir = ReportDir + "/VerificationRoutines/CSV-Compare/ResultReport/Dymola/"
...
def modelsCompare(modelList, modelType, simulationType):
    ...
    for modelNumber, modelName in enumerate(modelList):  
        totalTests += 1
        DymolaDir = modelDir + modelName+".csv"
        PSSE = PSSEDir + "/WorkingDir/"+simulationType+"/"+modelType+"/"+modelName+".csv"
        Report = ReportDir + simulationType+"/"+modelType+"/"
        compare = os.system("mono "+compareDir+"/Compare.exe --override --mode CsvFileCompare --delimiter " +comma+ " --reportdir " + Report + " --tolerance 1e-2 " +DymolaDir+ " " +PSSE)
        if compare == 0:
            testsPassed += 1
        else:
            testsFailed += 1
