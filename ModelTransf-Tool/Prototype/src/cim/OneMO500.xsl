...
model GEN</xsl:text>
	...
	<xsl:template match="cim:SynchronousMachineSimplified">
		...
		<xsl:value-of select="format-number(cim:RotatingMachineDynamics.saturationFactor,'0.000#')"/>
		<xsl:text>, S12 = </xsl:text>
		<xsl:value-of select="format-number(cim:RotatingMachineDynamics.saturationFactor120,'0.000#')"/>
		<xsl:text>);
		equation
			connect(</xsl:text>
